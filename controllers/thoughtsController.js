...
module.exports = {
  ...
  getSingleThought(req, res) {
    Thought.findOne({ _id: req.params.thoughtId })
      .select("-_v")
      .then((thought) => (!thought ? res.status(404).json({ message: "No ID associated with that thought !!" }) : res.json(thought)))
      .catch((err) => res.status(500).json(err));
  },
  ...
  createThought(req, res) {
    Thought.create(req.body)
      .then((thought) => {
        User.findByIdAndUpdate({ username: req.body.username }, { $push: { thoughts: thought._id } }).then((user) => {
          res.json(thought);
        });
      })
      .catch((err) => res.status(500).json(err));
    ...
    Thought.findOneAndDelete({ _id: req.params.thoughtId })
      .then((user) =>
        !user
          ...
          : User.findByIdAndUpdate({ username: user.username }, { $pull: { thoughts: user._id } })
      )
